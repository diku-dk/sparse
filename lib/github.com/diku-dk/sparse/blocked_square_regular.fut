...
module type blocked_square_regular = {
  ...
  -- | `blu_fill a` returns a sparse blocked matrix representing an
  -- LU-decomposition of `a`, assuming no fill-ins will occur.
  val blu_nofill [n] : mat[n] -> mat[n]

  val blk_mvm    [n] : [n][n]t -> [n]t -> [n]t
  val lu_dense   [n] : [n][n]t -> [n][n]t
       ...
       : blocked_square_regular with t = T.t =
{
  ...
  type~ mat [n] =
    ?[nz].{                     -- nz is the number of non-zero blocks
       n:[n](),                 -- n is dimension (a multiple of bsz)
       idxs : [nz]i64,          -- Non-zero block indexes (row-major)
       blks : [nz][bsz][bsz]t   -- As an invariance, we assume bsz divides n
     }
  ...
  def setop [m][n] (a:[m]i64) (b:[n]i64) : [m+n](i64,i64,i64) =
    ...
    in c
       ...
       |> scan (\(x,a1,b1) (y,a2,b2) ->
                  if x == y then (x,i64.max a1 a2, i64.max b1 b2)
                  else (y,a2,b2)) (0,-1,-1)

  def union [m][n] (a:[m]i64) (b:[n]i64) : ?[k].[k](i64,i64,i64) =
    let vs = setop a b
    in filter (\((v,_,_),i) ->
                 if i < m+n-1 && vs[i+1].0 == v then false
                 else true)
              (zip vs (iota (m+n)))
       |> map (.0)
  ...
  def binop_union [n] (f : t -> t -> t) (m1:mat[n]) (m2:mat[n]) : mat[n] =
    ...
    let blks = map (\(_,i,j) ->
                      if i >= 0
                      then if j >= 0 then map2 (map2 f) m1.blks[i] m2.blks[j]
                           else m1.blks[i]
                      else map (map (f zero_t)) (m2.blks[j])) xs
    let idxs = map (.0) xs
    in {n=m1.n,
        blks=blks,
        idxs=idxs}
  ...
  def smsmm [n] (a:mat[n]) (b:mat[n]) : mat[n] =
    ...
    let fs = map (\i -> if i == 0 || js[i] != js[i-1]
                        then 1 else 0) (indices js)
    let k = reduce (+) 0 fs
  ...
  -- it seems "backsolve' y U = forsolve (transpose U) y" !!
  ...
  def blu_nofill [n] (a:mat[n]) : mat[n] =
    ...
    let hrcbs = map3 (\h i b -> let (r,c) = idx_unflatten nb i
                                in (h,r,c,b))
                     (iota nz) idxs blks
    let hrcbs =
      loop hrcbs for i < nb do
      let blks = filter (\(_,r,c,_) -> r >= i && c >= i) (copy hrcbs)
      ...
      in assert (r == i && c == i)
                (let A21 = filter (\(_,r,c,_) -> r > i && c == i) blks
                 ...
                 let X21 = map (\(h,r,_,a) -> (h,r,map (\aa -> backsolve' aa b) a)) A21
                 let X12 = map (\(h,_,c,a) -> (h,c,transpose(map (\aa -> forsolve b aa) (transpose a)))) A12
                 let hrcbs[h] = (h,r,c,b)
                 let hrcbs =
                   loop hrcbs = hrcbs for j < length X21 do
                   let (h,r,b) = X21[j]
                   let hrcbs[h] = (h,r,c,b)
                   in hrcbs
                 let hrcbs =
                   loop hrcbs = hrcbs for j < length X12 do
                   let (h,c,b) = X12[j]
                   let hrcbs[h] = (h,r,c,b)
                   in hrcbs
                 let D' = map (\(_,r,A) ->
                 ...
                 let D = filter (\(_,r,c,_) -> r > i && c > i) blks
                         |> map (\(h,r,c,b) -> (h,idx_flatten nb (r,c),r,c,b))
                 let D'' = setops.join_by_key (.1) (.0) D D'                   -- ignore fillins
                           |> map (\((h,_,r,c,b),(_,_,_,b')) -> (h,r,c,matsub b b'))
                 let hrcbs =
                   loop hrcbs = hrcbs for j < length D'' do
                   let (h,r,c,b) = D''[j]
                   let hrcbs[h] = (h,r,c,b)
                   in hrcbs
                 in hrcbs)
